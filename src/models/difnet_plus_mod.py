...
class DiffnetPlusMod(tf.keras.Model):
    ...
    def call(self, inputs, training=False):
        ...
        while current_gcn_layer< self.gcn_layers:
            ...
            # convert to sparse tensor
            item_neighbors_sparse_matrix = tf.SparseTensor(indices=self.item_links['indices'], values=item_neighbors_values, dense_shape=[self.num_items, self.num_items])
            # softmax values
            ...
            item_gcn_embedding = 0.5 * current_item_gcn_embeddings + 0.5 *(item_consumed_users_attention_weight*item_embeddings_from_consumed_users + item_neighbors_attention_weight*item_embeddings_from_item_links) 
